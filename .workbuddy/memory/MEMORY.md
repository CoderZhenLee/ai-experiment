...
- 推送节奏(4 个自动化,prompt 内置 PushPlus 推送,均指向 /Users/lizhen/ai-experiment):
  ...
  - 周末 15:35 周末投资周总结(automation-1786429518927,**已改为 FR 15:35**,原 SA/SU 2026-08-12 改——用户反馈周三不该发,改为周五唯一触发,prompt 第一句也同步改为「今天是周五(本周最后一个交易日),A股已收盘」;原 SA/SU 期间曾被手动误触两次(8/11 周二、8/12 周三))
  - 用户要求:每条推送末尾都带网站链接(放最下面);分工明确不重复,15:35 日报为保底完整版
  - (曾建 09:35/11:35 盘中快报,用户 2026-08-11 要求精简,已删除)
  - 用户偏好(2026-08-11):所有推送**每条单独成行**,禁止一行塞多个条目(尤其前3名、ETF 宽基/行业要逐行)。4 个推送自动化 prompt 已全部加「排版要求」:盘中快报 1786416869322 / 学堂 1786416869287 / 日报 1785918218488 / 周总结 1786429518927
  - ⚠️ 关键坑:PushPlus markdown 模板会把「单个换行」当空格吞掉→多行内容挤成一行。已在 push_wechat.py 加 hardbreak_lines() 把段内单换行转 `<br>`,正文文件里正常换行即可
  - ⚠️ 手动重测推送时,标题别复用定时推送的时段名(如「盘中快报 · 14:00」),否则用户在非时段收到会困惑;测试用「格式测试(请忽略)」之类标题
